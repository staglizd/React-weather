...
const Detail = ({ label, data }) => (
    <div style={{ fontSize: '8rem' }}>
        <label style={{ color: COLORS.lightColor, paddingRight: '.25rem'}}>
            {label} :
        </label>
        <span>{data}</span>
    </div>
...
export default class UserCard extends Component {
    ...
    render() {
        ...
        if (_token.expiresAt === Infinity) {
            ...
            expiration = `${day}.${month}.${year} ${hour}:${minute}`
        ...
        const styles = {
            ...
            const avatar = _profile.profilePicURL ||
                'ttps://maxcdn.icons8.com/Share/icon/p1em/users//gender_neutral_user1600.png'
